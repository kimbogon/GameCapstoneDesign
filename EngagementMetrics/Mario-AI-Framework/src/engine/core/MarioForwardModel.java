...
public class MarioForwardModel {
    ...
    // 추가 
    private int cumulativeJumpFrames = 0; // 마리오가 점프한 프레임 수
    private int totalFrames = 0; // 전체 프레임 수
    ...
    public void advance(boolean[] actions) {
        ...
        this.totalFrames++;  // 매 프레임마다 totalFrames를 증가시킴
        trackJumpFrames(); 
    }
    ...
    // 마리오가 점프 상태라면 cumulativeJumpFrames를 증가시킴
    private void trackJumpFrames() {
        if (this.world.mario.jumpTime > 0) {
            cumulativeJumpFrames++;
        } 
    }

    // 전체 프레임 중 마리오가 점프한 프레임의 비율
    public float getJumpTimeRatio() {
        if (totalFrames == 0) return 0f;
        return (float) cumulativeJumpFrames / totalFrames;
    }
}
